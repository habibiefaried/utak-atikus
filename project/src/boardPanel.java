...
public class boardPanel extends JPanel {
    static final boolean USEVEC = true; // multiple painting per square (assume
                                        // user paints each default)
    int xdim, ydim;
    ...
    boardObject[][] board; // objects on board, listed as array of boardObject
                           // elements
    Vector boardVec; // objects on board to paint, listed as vector in order of
                     // painting
    ...
    public boardPanel(boardObject d, int x, int y) {
        this(d, 1, 1, x, y);
    }
    ...
    public boolean setSquare(boardObject b, Dimension d) {
        return setSquare(b, d.width, d.height);
    }
    ...
    public String toString() {
        ...
        for (int j = 0; j < ydim; j++) {
            retString += "[";
            if (board[0][j] != null)
                retString += board[0][j].toString();
            else
                retString += "x";
            for (int i = 1; i < xdim; i++) {
                retString += ",";
                if (board[i][j] != null)
                    retString += board[i][j].toString();
                else
                    retString += "x";
            }
            retString += ("]\n");
